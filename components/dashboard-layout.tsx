  ...
  return (
    <div className="relative group">
      <Link
        ...
        onClick={(e) => {
          if (module.subItems && module.subItems.length > 0 && !isCollapsed) {
            e.preventDefault()
            onToggle?.()
          }
        }}
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // 根据当前路径确定激活的模块和子模块
  const getActiveModuleAndSubModule = () => {
    for (const module of modules) {
      // 检查是否是主模块页面
      if (pathname === module.href) {
        return { activeModule: module.title, activeSubModule: null }
      }
      ...
      if (module.subItems) {
        for (const subItem of module.subItems) {
          if (pathname === subItem.href) {
            return { activeModule: module.title, activeSubModule: subItem.title }
          }
        }
      }
    }
    ...
    // 默认返回数据中心
    return { activeModule: "数据中心", activeSubModule: null }
  }

  const { activeModule, activeSubModule } = getActiveModuleAndSubModule()

  // 根据当前路径自动展开相应的模块
  useEffect(() => {
    const currentModule = modules.find(
      (m) => pathname === m.href || (m.subItems && m.subItems.some((sub) => pathname === sub.href)),
    )

    if (currentModule && currentModule.subItems && currentModule.subItems.length > 0) {
      setOpenModules((prev) => {
        if (!prev.includes(currentModule.title)) {
          return [...prev, currentModule.title]
        }
        return prev
      })
    }
